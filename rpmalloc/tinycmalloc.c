...
//! Current thread heap
static tls_t _memory_thread_heap;
...
static FORCEINLINE void *atomic_load_ptr(atomicptr_t *src) { return c89atomic_load_explicit_64(src, memory_order_relaxed); }
static FORCEINLINE void atomic_store_ptr(atomicptr_t *dst, void *val) { c89atomic_store_explicit_64(dst, val, memory_order_relaxed); }
static FORCEINLINE void atomic_store_ptr_release(atomicptr_t *dst, void *val) { c89atomic_store_explicit_64(dst, val, memory_order_release); }
static FORCEINLINE void *atomic_exchange_ptr_acquire(atomicptr_t *dst, void *val) { return c89atomic_exchange_explicit_64(dst, val, memory_order_acquire); }
static FORCEINLINE int atomic_cas_ptr(atomicptr_t *dst, void *val, void *ref) { return c89atomic_compare_exchange_weak_explicit_64(dst, &ref, val, memory_order_relaxed, memory_order_relaxed); }
...
#if defined(_WIN32)
static void _rpmalloc_thread_destructor(void *value) {
...
_rpmalloc_global_cache_finalize(global_cache_t * cache) {
    ...
    for (ispan = 0; ispan < cache->count; ++ispan)
...
_rpmalloc_global_cache_insert_spans(span_t * *span, size_t span_count, size_t count) {
    ...
    span_t *keep = 0;
    size_t ispan;
    for (ispan = insert_count; ispan < count; ++ispan) {
...
#else
            for (ispan = 0; ispan < THREAD_SPAN_CACHE_TRANSFER; ++ispan)
...
#else
            for (ispan = 0; ispan < transfer_count; ++ispan)
                _rpmalloc_span_unmap(span_cache->span[remain_count + ispan]);
#endif
            span_cache->count = remain_count;
        }
...
void *rpaligned_realloc(void *ptr, size_t alignment, size_t size, size_t oldsize,
                        unsigned int flags) {
    heap_t *heap = get_thread_heap();
...
inline int rpposix_memalign(void **memptr, size_t alignment, size_t size) {
    if (memptr)
...
inline size_t rpmalloc_usable_size(void *ptr) {
    return (ptr ? _rpmalloc_usable_size(ptr) : 0);
}

inline void rpmalloc_thread_collect(void) {}
...
void rpmalloc_dump_statistics(void *file) {
    ...
    for (list_idx = 0; list_idx < HEAP_ARRAY_SIZE; ++list_idx) {
        ...
        while (heap) {
            int need_dump = 0;
            for (iclass = 0; !need_dump && (iclass < SIZE_CLASS_COUNT); ++iclass) {
                if (!atomic_load32(&heap->size_class_use[iclass].alloc_total)) {
            ...
            for (iclass = 0; !need_dump && (iclass < LARGE_CLASS_COUNT); ++iclass) {
                if (!atomic_load32(&heap->span_use[iclass].high) && !atomic_load32(&heap->span_use[iclass].spans_map_calls))
                    continue;
                need_dump = 1;
            }
