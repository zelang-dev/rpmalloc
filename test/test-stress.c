...
static void* alloc_items(size_t items, random_t r) {
  ...
  if (items==0) items = 1;
  uintptr_t i, * p = (uintptr_t*)custom_calloc(items,sizeof(uintptr_t));
  if (p != NULL) {
    for (i = 0; i < items; i++) {
      assert(p[i] == 0);
      p[i] = (items - i) ^ cookie;
...
#else
static void *atomic_exchange_ptr(volatile void **p, void *newval) {
    return (void *)atomic_exchange(p, newval);
}
#endif
